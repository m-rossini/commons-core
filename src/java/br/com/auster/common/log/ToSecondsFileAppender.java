...
//import static br.com.auster.common.log.UniqueDatedFileAppender.datestamp;
//import static br.com.auster.common.log.UniqueDatedFileAppender.tomorrow;
 ...
 */
public class ToSecondsFileAppender extends FileAppender {

	// ----------------------------------------------------- Instance Variables

	/**
	 * The directory in which log files are created. Wihtout a leading slash, this
	 * is relative to the Tomcat home directory.
	 */
	 ...
	 * </ul>
	 */
	public ToSecondsFileAppender() {
		aThreadId = Long.toString(Thread.currentThread().getId()) ;
	}
	 ...
	 *          the suffix that is added to log file filenames.
	 */
	public ToSecondsFileAppender(String directory, String prefix, String suffix) {
		aThreadId = Long.toString(Thread.currentThread().getId()) + separator + (this.hashCode() + Calendar.getInstance().getTimeInMillis());
	...
	public boolean getUseThreadId() {
		return this.useThreadId;
	}
	
	public void setUseThreadId(boolean useThreadId) {
		this.useThreadId = useThreadId;
	}
	// ------------------------------------------------------------- Properties
	/**
	...
	public void append(LoggingEvent event) {
		if (this.layout == null) {
			errorHandler
					.error("No layout set for the appender named [" + name + "].");
			return;
		}
		if (this.m_calendar == null) {
			errorHandler.error("Improper initialization for the appender named ["
					+ name + "].");
			return;
		}
		...
		if (n >= m_tomorrow) {
			...
			String name = m_prefix  + datestamp ;
			if (this.useThreadId) {
				name += separator + aThreadId;
			}
			
			File newFile = new File(m_path, name  + m_suffix);
		...
		if (this.qw == null) { // should never happen
			errorHandler
					.error("No output stream or file set for the appender named [" + name
							+ "].");
			return;
		}
		subAppend(event);
	 ...
	 * mercifully accommodating in handling non-existent dates. For example, June
	 * 31 is understood to mean July 1. This allows you to simply add one to
	 * today's day of the month to generate tomorrow's date. It also works for
	 * years, so that December 32, 2004 is converted into January 1, 2005.
	 * </p>
	 */
